package pl.wondermc.boxpvp.command.framework;
...
public class CommandFramework implements CommandExecutor {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if(sender instanceof Player){
					...
					if(!sender.hasPermission(command.permission())){
						sender.sendMessage(ChatHelper.fixColor("&cNie posiadasz permisji aby uzyc tej komendy!"));
						return true;
					}
				}
				try {
	...
	private void defaultCommand(CommandArgs args) {
		args.getSender().sendMessage(ChatHelper.fixColor("&cBlad: &7" + args.getLabel() + "&c! Zglos to do administracji!"));
	}
